...
<head>
<meta charset="UTF-8">
<title>Heitor vs Tina - Edição Sonora</title>
<style>
  body {
    margin: 0;
    overflow: hidden;
  ...
  #tina, #luiza, #laura, #tinatriste, #vodidi, #vodeco, #tiad, #tioguigo, #violao {
    position: absolute;
    image-rendering: pixelated;
  }
  ...
  /* ===== ESTILOS PARA VÓ DIDI E CHURROS ===== */
...
<div id="menu-screen">
  <div class="menu-container">
    <button class="menu-btn" onclick="iniciarAventura()">🎮 Começar Aventura</button>
    <button class="menu-btn" onclick="abrirConsideracoes()">📜 Considerações</button>
    <button class="menu-btn" onclick="abrirCreditos()">🎬 Créditos</button>
    <button class="menu-btn" id="menu-sair" onclick="sairJogo()">❌ Sair</button>
  </div>
</div>
...
<div id="consideracoes-modal">
  <div class="modal-content">
    <h2>💖 Considerações 💖</h2>
    <p>Este jogo trata-se de algo feito para nosso amado Menino Heitor, se lembrar eternamente de todos aqueles que o amam e o quererm bem. Algo simples, mas feito com amor e que será uma memoria afetiva poderosa. Te amamos Heitor, Aproveitem cada momento desse pequeno entretenimento.</p>
...
<div id="game-screen">
  <div id="pokemon-intro">
    <div class="pokemon-container">
      ...
      <button class="duelar-btn" id="duelar-btn">⚡ DUELAR ⚡</button>
    </div>
  </div>
  ...
  <!-- CAIXA DE INTERAÇÃO COM NPCs -->
  <div id="interaction-prompt">
    🔶 APERTE [E] PARA INTERAGIR
  </div>

  <!-- MENU DE ESCOLHA (SIM/NÃO) -->
  <div id="choice-menu">
    <h3>🍪 VÓ DIDI DIZ 🍪</h3>
    <p>Você DEVE estar cansado de suas viagens! Venha, desfrute de um delicioso churros!!</p>
    <div class="choice-buttons">
      <button class="choice-btn" onclick="escolherChurros('sim')">Hummm, delícia! Quero um!</button>
      <button class="choice-btn" onclick="escolherChurros('nao')">Não estou a fim...</button>
    </div>
  </div>
  ...
  <div id="churros-menu">
    <div id="churros-menu-content">
      <h1 class="churros-title">🍪 CARDÁPIO DE CHURROS 🍪</h1>
      ...
      <div class="churros-container">
        ...
        <div class="churro-option" onclick="escolherChurro('doce')">
          ...
          <div class="churro-info">
            <div class="churro-name">🍯 CHURRO DOCE DE LEITE 🍯</div>
            <p class="churro-description">
              Um churro irresistível recheado com doce de leite cremoso e envolvido em calda de doce de leite puro. Crocante por fora, derretendo o melhor doce de leite por dentro. Uma verdadeira explosão de sabor que vai deixar seu paladar pedindo mais!
            </p>
            <button class="churro-btn">Escolher este 😍</button>
          </div>
        </div>
        ...
        <div class="churro-option" onclick="escolherChurro('nutella')">
          ...
          <div class="churro-info">
            <div class="churro-name">🍫 CHURRO NUTELLA 🍫</div>
            <p class="churro-description">
              Um espetáculo de sabor! Churro crocante recheado com Nutella suíça pura, derretida na textura perfeita. Finalizado com um toque de calda de chocolate belga e avelã. Uma experiência de sabor inigualável que fará você voltar sempre!
            </p>
            <button class="churro-btn">Escolher este 🤤</button>
          </div>
        </div>
    ...
    <button class="churro-decline-btn" onclick="recusarChurrosAposMenu()">Mudei de ideia... 😅 Sair do Menu</button>
  </div>

  <!-- TELA DO VIOLÃO -->
  <div id="violao-screen">
    <button class="violao-close-btn" onclick="fecharTelaViolao()">✕</button>
    <div class="violao-container">
      <img src="hviolao.png" id="heitor-violao" alt="Heitor com Violão">
      <button class="btn-solta-som" onclick="tocandoViolao()">🎸 SOLTA O SOM! 🎸</button>
    </div>
  </div>

  <!-- INVENTÁRIO -->
  <button id="inventory-button" onclick="abrirInventario()">📦 INVENTÁRIO [I]</button>

  <div id="inventory-panel">
    <button id="inventory-close" onclick="fecharInventario()">✕</button>
    <div class="inventory-title">📦 INVENTÁRIO</div>
    <div class="inventory-grid" id="inventory-grid"></div>
    <div class="inventory-description" id="inventory-description">Selecione um item para ver a descrição</div>
  </div>

  <!-- VISUALIZADOR DE ITEM -->
  <div id="item-viewer">
    <button class="item-viewer-close" onclick="fecharViewerItem()">✕</button>
...
<script>
  ...
  // ÁUDIO - Inicializar sem loop inicialmente
  ...
  // FLAG para controlar se a música de menu já tocou
  let musicaMenuTocando = false;

  // INICIAR MÚSICA DO MENU - apenas uma vez
  function iniciarMusicaMenu() {
    if (!musicaMenuTocando) {
      musicaMenuTocando = true;
      somMenu.play().catch(e => console.log("Música do menu ainda não disponível"));
  ...
  const itemDescriptions = {
    'carta': 'Item que a vovó do churros me deu. Parece usada.',
    'violao': 'Violão que ganhei do vovó músico! Parece usado.',
  ...
  function iniciarAventura() {
    ...
    // Parar COMPLETAMENTE a música do menu
    somMenu.pause();
    ...
    setTimeout(() => {
      ...
      somPrincipal.play().catch(e => console.log("Música principal ainda não disponível"));
      nextText();
    }, 100);
  }
  ...
  function nextText() {
    ...
    if (stage === 0) {
      text.innerText = "Heitor precisa de você.";
      stage = 1;
      setTimeout(() => { heitorContainer.style.left = "40%"; }, 500);
    } else if (stage === 1) {
      text.innerText = "A jornada começa agora...";
  ...
  function showMission() {
    const box = document.getElementById("dialog-box");
    box.style.display = "block";
    document.getElementById("dialog-title").innerText = "⚠️ ATENÇÃO!";
    document.getElementById("dialog-text").innerText = "A Tina está querendo arranhar a Princesa Luiza, salve-a!";
    document.getElementById("dialog-hint").style.display = "none";
    
    const startListener = (e) => {
      if (e.key.toLowerCase() === 'e') {
        // Parar música principal completamente
        somPrincipal.pause();
        somPrincipal.currentTime = 0;
        
        // Aguardar e começar a música de batalha
        setTimeout(() => {
          somBatalha.currentTime = 0;
          somBatalha.play().catch(e => console.log("Música de batalha ainda não disponível"));
          box.style.display = "none";
  ...
  function enableControls() {
    document.addEventListener("keydown", e => {
      ...
      if ((e.code === "Space" || e.key === "w") && !isJumping && gameActive) {
        ...
        somPulo.play().catch(e => console.log("Som de pulo não disponível"));
      }
    });
  ...
  function attack() {
    ...
    somAtaque.play().catch(e => console.log("Som de ataque não disponível"));
  ...
  function startVictoryCutscene() {
    ...
    setTimeout(() => {
      somPrincipal.currentTime = 0;
      somPrincipal.play().catch(e => console.log("Música principal não disponível"));
    }, 100);
  ...
  async function showFinalDialog() {
    direction = -1;
    updateVisuals();
    await showDialog("PRINCESA LUIZA DIZ:", "Heitor! Obrigado por me salvar. Agora precisam de você em outro lugar. Vá! Corra!");
    luizaEscape();
  }
  ...
  async function showLevel2Intro() {
    const box = document.getElementById("dialog-box");
    box.style.display = "block";
    document.getElementById("dialog-title").innerText = "🏞️ NOVA ÁREA";
    document.getElementById("dialog-text").innerText = "Heitor avançou! Mas sua aventura não termina aqui...";
    document.getElementById("dialog-hint").style.display = "none";
  ...
  async function showLauraFirstDialog() {
    await showDialog("LAURA DIZ:", "Como ousa bater na minha GATA!!! Você sofrerá.");
    showLauraSecondDialog();
  }

  async function showLauraSecondDialog() {
    await showDialog("LAURA DIZ:", "Vou fazer cocegas em você. VAMOS DUELAR!!");
    showPokemonIntro();
  }
  ...
  function iniciarJokenpo() {
    ...
    somjokenpo.play().catch(e => console.log("Som jokenpo não disponível"));
  ...
  function determinarVencedor(h, l) {
    ...
    if ((h === 'pedra' && l === 'tesoura') ||
      ...
      resultsDiv.textContent = "HEITOR VENCE! 🎉";
      somcocegas.currentTime = 0;
      somcocegas.play().catch(e => console.log("Som cocegas não disponível"));
      aplicarCocegasLaura();
    } else {
      jokenpoLauraVitorias++;
      resultsDiv.textContent = "LAURA VENCE! 💪";
      somcocegas.currentTime = 0;
      somcocegas.play().catch(e => console.log("Som cocegas não disponível"));
  ...
  async function showHeitorVictoryScreen() {
    await showDialog("LAURA DIZ:", "Você teve sorte! Te deixo passar.");
  ...
  async function showVodidiDialog() {
    await showDialog("VOVÓ DIDI DIZ:", "Olá, nobre guerreiro! Aproxime-se.");
    gameActive = true;
    gameLoop();
  }
  ...
  function gameLoop() {
    ...
    if (level5Active && tiaDInteractionCompleted && !tiaDInteractionActive) {
      verificarProximidadeTiaDInteracao();
    }

    if (level5Active && tioGuigoInteractionCompleted === false) {
    ...
    if (level5Active && exitArrowVisible) {
      verificarSaidaLevel5();
    }
    
    if ((battleActive && !level2Active) || level2Active || level3Active || level4Active || level5Active) {
      if (keys["a"] || keys["A"]) { 
        velocityX -= accel; 
        direction = -1; 
      }
  ...
  function escolherChurros(opcao) {
    ...
    if (opcao === 'sim') {
      setTimeout(() => {
        showDialog("VÓ DIDI DIZ:", "Que ótimo! Olhe o cardápio.").then(() => {
          document.getElementById("churros-menu").style.display = "flex";
        });
      }, 300);
    } else if (opcao === 'nao') {
      setTimeout(() => {
        showDialog("VÓ DIDI DIZ:", "Hummm... você tem certeza?").then(() => {
  ...
  function mostrarConfirmacaoChurros() {
    ...
    p.innerText = "Tem certeza de que não quer?";
    ...
    btn2.onclick = () => {
      choiceMenu.style.display = "none";
      setTimeout(() => {
        showDialog("VÓ DIDI DIZ:", "Pelo amor da Quitá! Que pena.").then(() => {
  ...
  function escolherChurro(tipo) {
    ...
    somComer.play().catch(e => console.log("Som comer não disponível"));
    
    if (tipo === 'doce') {
      setTimeout(() => {
        showDialog("HEITOR COMEU:", "Ahhh, que delícia! Este doce de leite é simplesmente perfeito! 😍").then(() => {
          finalizarChurrosEvent();
        });
      }, 300);
    } else if (tipo === 'nutella') {
      setTimeout(() => {
        showDialog("HEITOR COMEU:", "Meu Deus! Este churro de Nutella é SENSACIONAL! Que sabor indescritível! 🤤").then(() => {
          finalizarChurrosEvent();
        });
      }, 300);
    }
  ...
  function recusarChurrosAposMenu() {
    document.getElementById("churros-menu").style.display = "none";
    setTimeout(() => {
      showDialog("HEITOR DISSE:", "Vou recusar... desculpa, Vó Didi!").then(() => {
        finalizarChurrosEvent();
      });
    }, 300);
  }
  ...
  function receberCarta() {
    setTimeout(() => {
      showDialog("VÓ DIDI DIZ:", "TOME PARA VOCÊ ISTO, EU GOSTEI MUITO DE VOCÊ JOVEM, LEMBRA MEU NETO.").then(() => {
  ...
  function mostrarNotificacaoInventario() {
    ...
    notification.innerHTML = "✨ INVENTÁRIO DESBLOQUEADO! Pressione [I] para abrir";
  ...
  function iniciarInteracaoVodeco() {
    ...
    if (distancia < 200) {
      ...
      showDialog("VÓ DECO DIZ:", "Olá jovem guerreiro! Tudo bem?").then(() => {
        showDialog("VÓ DECO DIZ:", "Somos nobres andarilhos e adoramos música! Você sabe tocar?").then(() => {
  ...
  function mostrarOpcoesMusicaVodeco() {
    ...
    h3.innerText = "🎸 VÓ DECO PERGUNTA 🎸";
    p.innerText = "Você sabe tocar?";
    choiceMenu.style.display = "block";
  ...
  function pegarViolao(resposta) {
    ...
    if (resposta === 'sim') {
      setTimeout(() => {
        showDialog("VÓ DECO DIZ:", "Pegue o violão!").then(() => {
      ...
      setTimeout(() => {
        showDialog("VÓ DECO DIZ:", "Humm, mas pegue mesmo assim, fica bem em você.").then(() => {
          violaoDesbloqueado = true;
          telaViolaoAberta = true;
          somPrincipal.pause();
          somPrincipal.currentTime = 0;
  ...
  function atualizarBotaoViolao(tipo) {
    ...
    if (tipo === 'sim') {
      btn.innerText = "🎸 SOLTA O SOM! 🎸";
      btn.onclick = () => tocandoViolao();
    } else if (tipo === 'nao') {
      btn.innerText = "Você não sabe tocar... 😅";
  ...
  function tocandoViolao() {
    ...
    somviolao.play().catch(e => console.log("Som violão não disponível"));
  ...
  function fecharTelaViolao() {
    ...
    setTimeout(() => {
      if (violaoRespostaUsuario === 'sim') {
        showDialog("VÓ DECO DIZ:", "Nossa! Você toca muito bem.").then(() => {
          showDialog("VÓ DECO DIZ:", "Toca como meu filho! Fique com o violão.").then(() => {
            finalizarViolaoEvent();
          });
        });
      } else if (violaoRespostaUsuario === 'nao') {
        showDialog("VÓ DECO DIZ:", "Que pena, mas fique para você, quem sabe aprende a tocar como meu filho!").then(() => {
          finalizarViolaoEvent();
        });
      }
    }, 300);
  ...
  function finalizarViolaoEvent() {
    ...
    const notification = document.getElementById("inventory-notification");
    notification.innerHTML = "✨ VIOLÃO ADICIONADO AO INVENTÁRIO!";
    notification.style.display = "block";
    document.getElementById("inventory-button").style.display = "block";
  ...
  // ========== NÍVEL 5 - TABERNA COM TIA D E TIO GUIGO ==========
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          somTaberna.play().catch(e => console.log("Música da taberna ainda não disponível"));
        }, 100);
  ...
  async function iniciarDialogoInicial() {
    await showDialog("TIA ANDREZA DIZ:", "Olá jovem guerreiro! Bem vindo a minha taberna.");
  ...
  function iniciarInteracaoTiaD() {
    ...
    if (distancia < 200) {
      ...
      showDialog("TIA ANDREZA DIZ:", "Queria cigarros melhores para fumar...").then(() => {
        showDialog("TIA ANDREZA DIZ:", "Ei! Você é muito novo para fumar, eu não deveria falar disso com você.").then(() => {
  ...
  function iniciarInteracaoTioGuigo() {
    ...
    if (distancia < 200) {
      ...
      showDialog("TIO GUIGO DIZ:", "Funcionários! Eu estou tão irritado!").then(() => {
        showDialog("TIO GUIGO DIZ:", "Ó que! Desculpe garoto, não te vi ai.").then(() => {
          showDialog("TIO GUIGO DIZ:", "Esses funcionarios me tiram do serio! Hoje em dia é tão difícil ganhar um seudo dinheiro.").then(() => {
  ...
  function mostrarPerguntaPerfumeGuigo() {
    ...
    h3.innerText = "👃 TIO GUIGO DIZ 👃";
    p.innerText = "Garoto, você está cheirando mal... O que acha de um perfume?";
    choiceMenu.style.display = "block";
  ...
  function escolherPerfumeGuigo(opcao) {
    ...
    if (opcao === 'sim') {
      setTimeout(() => {
        showDialog("TIO GUIGO DIZ:", "Tome, você tem sorte que eu tenho uma fragrância extra aqui.").then(() => {
      ...
      setTimeout(() => {
        showDialog("TIO GUIGO DIZ:", "Você tem certeza?? Tenho um perfume ótimo para você! É por conta da casa.").then(() => {
  ...
  function mostrarConfirmacaoPerfumeGuigo() {
    ...
    const btn1 = document.createElement("button");
    btn1.className = "choice-btn";
    btn1.innerText = "Tá bem, vou aceitar!";
    btn1.onclick = () => {
      choiceMenu.style.display = "none";
      setTimeout(() => {
        showDialog("TIO GUIGO DIZ:", "Ótimo! Quando precisar estou aqui.").then(() => {
          inventoryItems[2] = 'perfume';
          mostrarNotificacaoPerfume();
          finalizarInteracaoTioGuigo();
        });
    ...
    const btn2 = document.createElement("button");
    btn2.className = "choice-btn";
    btn2.innerText = "Não, tenho certeza!";
    btn2.onclick = () => {
      choiceMenu.style.display = "none";
      setTimeout(() => {
        showDialog("TIO GUIGO DIZ:", "Ok, é de sua escolha feder!").then(() => {
          finalizarInteracaoTioGuigo();
        });
      }, 300);
    };
  ...
  function mostrarNotificacaoPerfume() {
    const notification = document.getElementById("inventory-notification");
    notification.innerHTML = "✨ PERFUME ADICIONADO AO INVENTÁRIO!";
    notification.style.display = "block";
  ...
  function finalizarJogo() {
    ...
    setTimeout(() => {
      ...
      showDialog("🏆 FIM DA AVENTURA 🏆", 
                 "Você completou uma jornada épica, Heitor!\n\nVocê enfrentou desafios, fez novos amigos e descobriu o verdadeiro valor da amizade e coragem.\n\nQue sua próxima aventura seja ainda maior!\n\nTE AMAMOS, HEITOR! 💖").then(() => {
  ...
  function voltarAoMenu() {
    ...
    somMenu.play().catch(e => console.log("Música do menu não disponível"));
  }
  ...
  function abrirInventario() {
    ...
    document.getElementById("inventory-description").innerText = "Selecione um item para ver a descrição";
  }
  ...
  // ========== EVENT LISTENERS ==========
    ...
    if (e.key.toLowerCase() === 'e') {
      ...
      if (level5Active) {
        iniciarInteracaoTiaD();
        iniciarInteracaoTioGuigo();
      }
    }
